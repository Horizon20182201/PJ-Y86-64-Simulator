...
3. 把 **单步执行逻辑** 集中在 `step(CPU&)` 中，实现“取指→译码→执行→访存→写回→PC 更新”一条龙
...
> 亮点：**取指和译码统一封装**，上层 `step()` 只需要看 `Decoded`，不会直接操作 `CPU.mem`。
...
> 特点：逻辑上模拟了 **五阶段流水线** 的行为，但实现上仍然是 **顺序执行的一次性函数**，结构清晰易读。
...
> 小不足：`CALL/PUSHQ` 在 Execute 阶段和 Writeback 阶段都写了一次 `rsp`，语义正确但职责略有重叠，可以再分清“只在一个阶段改写”。
...
从 `file` 字段可以看出，bench_y86.py 使用了一整套有代表性的 `.yo` 程序：
...
> 这些 benchmark 不是“真实应用”，而是针对不同指令模式的“小显微镜”。
...
* 增加统计：
  ...
  * 每轮的方差 / 标准差，刻画稳定性
  * 更长、更真实一点的程序（例如由你的 Mini-C 编译器生成的 `.yo`）
* 更细粒度的指标：
